...
<div class="flex flex-col md:flex-row gap-4 mb-6">
    <div class="flex-1 bg-purple-100 p-4 rounded-lg shadow-md">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Gestión de Agencias</h2>
        <p class="mt-1 text-sm text-gray-900 dark:text-gray-400">
            Listado completo de todas las agencias registradas en el sistema.
        </p>
    </div>

    <div class="flex items-center justify-center md:justify-end bg-purple-200 p-4 rounded-lg shadow-md">
        <a href="<?= base_url('agencias/new'); ?>">
    ...
    <div class="overflow-x-auto">
        <table id="tbl_agencias" class="min-w-full bg-white dark:bg-gray-800 dark:text-gray-300">
            ...
            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                ...
                <tr class="text-gray-700 dark:text-gray-400">
                    <td class="px-4 py-3 text-sm"><?= $agencia['id_agencia']; ?></td>
                    <td class="px-4 py-3 text-sm"><?= $agencia['nombre']; ?></td>
                    <td class="px-4 py-3 text-sm"><?= $agencia['telefono']; ?></td>
                    <td class="px-4 py-3 text-sm"><?= $agencia['ciudad']; ?></td>
                    <td class="px-4 py-3 text-sm max-w-xs truncate"><?= $agencia['direccion']; ?></td>
                    <td class="px-4 py-3 text-sm"><?= $agencia['horario_atencion']; ?></td>
                    <td class="px-4 py-3 text-sm max-w-xs truncate " ><?= $agencia['correo']; ?></td>
                    <td class="px-4 py-3 text-sm">
                        <?php if (!empty($agencia['foto'])): ?>
                            <img class="w-12 h-12 rounded-full object-cover" src="<?= base_url('uploads/agencias/' . $agencia['foto']); ?>" alt="Foto agencia" loading="lazy">
                        <?php else: ?>
                            <span class="text-gray-500 dark:text-gray-400">N/A</span>
                        <?php endif; ?>
                    </td>
                    <td class="px-4 py-3 text-sm"><?= $agencia['latitud']; ?></td>
                    <td class="px-4 py-3 text-sm"><?= $agencia['longitud']; ?></td>
                    <td class="px-4 py-3 text-sm">
                        <div class="flex items-center space-x-3">
                            <a href="<?= base_url('agencias/'.$agencia['id_agencia'].'/edit'); ?>" 
                            ...
                            <button onclick="eliminarAgencia('<?= base_url('agencias/'.$agencia['id_agencia']) ?>')" 
